...
func (s *Service) Execute(cmd interface{}) *models.Answer {
	...
	case models.CommandCreateCredentials:
		err := s.credsService.InsertCredentials(cmd.(models.CommandCreateCredentials).Credentials)
		return &models.Answer{
			Err: err,
		}
	case models.QueryReadCredentialsByUsername:
		credentials, err := s.credsService.GetCredentials(cmd.(models.QueryReadCredentialsByUsername).Username)
		return &models.Answer{
			Err:         err,
			Credentials: credentials,
		}
	case models.CommandDeleteCredentialsByUsername:
		err := s.credsService.DeleteCredentials(cmd.(models.CommandDeleteCredentialsByUsername).Username)
		return &models.Answer{
			Err: err,
		}
	case models.CommandCreateSession:
		err := s.sessionService.InsertSession(cmd.(models.CommandCreateSession).Session)
		return &models.Answer{
			Err: err,
		}
	case models.QueryReadSessionByRefreshToken:
		session, err := s.sessionService.GetSession(cmd.(models.QueryReadSessionByRefreshToken).RefreshToken)
		return &models.Answer{
			Err:     err,
			Session: session,
		}
	case models.CommandDeleteSessionByRefreshToken:
		err := s.sessionService.DeleteSession(cmd.(models.CommandDeleteSessionByRefreshToken).RefreshToken)
		return &models.Answer{
			Err: err,
		}
	default:
